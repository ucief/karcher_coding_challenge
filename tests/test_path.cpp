...
void test_preprocessing()
{
    ...
    const std::vector<Point> raw{{0, 0}, {0.001, 0}, {0.002, 0}, {0.1, 0}, {0.101, 0}, {0.102, 0}};
    ...
    const auto slow = preprocess_path({{0, 0}, {0.004, 0}, {0.008, 0}, {0.012, 0}, {0.016, 0}, {0.020, 0}});
    if (BOOST_TEST_EQ(slow.size(), 4u))
        check_near(slow[2].x(), 0.012);
...
{
    const auto path = preprocess_path({{-0.06, 0}, {-0.02, 0}, {0, 0}, {0.02, 0.02}, {0.06, 0.08}}, 0);
...
void test_heading_window()
{
    ...
    if (BOOST_TEST_EQ(local.size(), 4u))
        check_near(local[1].heading, 0);
}
...
void test_heading_edge_cases()
{
             ...
             {{0, 0}}, {{0, 0}, {0, 0}, {0, 0}}})
    {
        ...
        BOOST_TEST_EQ(trajectory.size(), path.size());
        for (const auto &pose : trajectory)
            BOOST_TEST(std::isnan(pose.heading));
    }
    // A path shorter than the required heading baseline cannot provide
    // a sufficiently reliable heading estimate.
    const auto short_path =
        estimate_trajectory({{0, 0.02}, {0, 0}});

    BOOST_TEST_EQ(short_path.size(), 2u);

    for (const auto &pose : short_path)
    {
        BOOST_TEST(std::isnan(pose.heading));
    }

    // Raw duplicates and the stationary tail are retained, with a stable heading.
    const std::vector<Point> raw{{0, 0}, {0, 0}, {0.1, 0}, {0.2, 0}, {0.201, 0.001}, {0.199, -0.001}, {0.2, 0}};
    const auto trajectory = estimate_trajectory(raw);
    BOOST_TEST_EQ(trajectory.size(), raw.size());
    for (const auto &pose : trajectory)
        check_near(pose.heading, 0, 0.02);
    ...
    BOOST_TEST_EQ(reversal.size(), 3u);

    for (const auto &pose : reversal)
    {
        BOOST_TEST(std::isnan(pose.heading));
    }
}
...
void test_curvature()
{
    ...
    for (double value : straight)
        check_near(value, 0);
    const auto turn = estimate_curvatures({{{0, 0}, 0}, {{0.5, 0}, -0.25}});
    if (BOOST_TEST_EQ(turn.size(), 1u))
        check_near(turn[0], 0.5);
...
void test_cleaned_area()
{
    ...
    const Trajectory separated{{{0, 0}, 0}, {{2, 0}, 0}, {{3, 0}, unknown}, {{4, 0}, 0}, {{6, 0}, 0}};
    ...
    const Trajectory revolution{{{0, 0}, 0}, {{0, 0}, pi / 2}, {{0, 0}, pi}, {{0, 0}, -pi / 2}, {{0, 0}, 0}};
