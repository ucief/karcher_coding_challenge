...
void add_sweep(std::vector<Polygon>& sweeps, Point a, Point b, Point c, Point d) {
    ...
    if (bg::area(sweep) > 1e-15) sweeps.push_back(std::move(sweep));
}
...
std::vector<Point> preprocess_path(const std::vector<Point>& path, double min_movement) {
    ...
    if (path.empty() || min_movement == 0) return path;
    ...
    while (first < path.size()) {
        ...
        if (last != first) result.push_back(path[last]);
        first = last + 1;
    }
    return result;
}
...
double estimate_heading(const std::vector<Point>& path, std::size_t index,
    ...
    if (index >= path.size()) throw std::out_of_range("Heading point index is outside the path");

    std::size_t left = index;
    std::size_t right = index;
    double longest = 0;
    double heading = std::numeric_limits<double>::quiet_NaN();
    while (left > 0 || right + 1 < path.size()) {
        if (left > 0) --left;
        if (right + 1 < path.size()) ++right;
        const double baseline = bg::distance(path[left], path[right]);
        if (baseline > longest) {
            longest = baseline;
            heading = std::atan2(path[right].y() - path[left].y(),
                                 path[right].x() - path[left].x());
        }
        if (baseline >= minimum_distance) break;
    }
    return heading;
}
...
double speed_model(double curvature) {
    ...
    if (curvature < 0.5) return 1.10;
    if (curvature >= 10.0) return 0.15;
...
double cleaned_area(const Trajectory& trajectory, const std::array<Point, 2>& gadget) {
    ...
    for (std::size_t i = 1; i < trajectory.size(); ++i) {
        const Pose& a = trajectory[i - 1];
        const Pose& b = trajectory[i];
        if (!std::isfinite(a.heading) || !std::isfinite(b.heading)) continue;
